...
export async function sendFacebookEvent(eventData: FacebookEventData): Promise<boolean> {
  try {
    ...
    if (!settings) {
      console.warn('Facebook tracking not configured, skipping event:', eventData.event_name)
      return false
    }
...
function getUserData(userInfo?: {
  ...
  return {
    em: userInfo.email ? simpleHash(userInfo.email.toLowerCase()) : undefined,
    ph: userInfo.phone ? simpleHash(userInfo.phone.replace(/[^0-9]/g, '')) : undefined,
    fn: userInfo.firstName ? simpleHash(userInfo.firstName.toLowerCase()) : undefined,
    ln: userInfo.lastName ? simpleHash(userInfo.lastName.toLowerCase()) : undefined,
    ct: userInfo.city ? simpleHash(userInfo.city.toLowerCase()) : undefined,
    st: userInfo.state ? simpleHash(userInfo.state.toLowerCase()) : undefined,
...
export async function trackPageView(url?: string, userInfo?: Parameters<typeof getUserData>[0]): Promise<boolean> {
...
export async function testFacebookConnection(): Promise<{ success: boolean; message: string }> {
  try {
    const settings = await getFacebookSettings()
    ...
    if (!settings) {
      return {
        success: false,
        message: 'Facebook settings not configured. Please add Pixel ID and Access Token.'
      }
    }
