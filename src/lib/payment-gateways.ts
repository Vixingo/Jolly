...
export async function initializeSSLCOMMERZ(
    ...
    try {
        ...
        const payload = {
            ...
            tran_id: paymentRequest.order_id,
            success_url: paymentRequest.success_url,
            fail_url: paymentRequest.fail_url,
            cancel_url: paymentRequest.cancel_url,
            cus_name: paymentRequest.customer_name,
            cus_email: paymentRequest.customer_email || "customer@example.com",
            cus_phone: paymentRequest.customer_phone,
            cus_add1: paymentRequest.customer_address,
            cus_city: "Dhaka",
        ...
        const response = await fetch(
            "https://noninursery.com/api/init-payment.php",
            {
                method: "POST",
                headers: {
        ...
        if (result.success) {
            ...
            return {
                success: false,
                error:
                    result.error || "SSLCOMMERZ payment initialization failed",
            };
        }
    } catch (error) {
...
// bKash Integration
export async function initializeBkash(
    paymentRequest: PaymentRequest
): Promise<PaymentResponse> {
    try {
        const settings = await getPaymentGatewaySettings();
