...
export async function sendGoogleAdsConversion(
    ...
    try {
        ...
        if (!label) {
            console.warn(
                "No conversion label provided for Google Ads conversion"
            );
            return false;
        }
...
export async function testGTMConnection(): Promise<{
    success: boolean;
    message: string;
}> {
    try {
        ...
        if (!settings) {
            return {
                success: false,
                message:
                    "GTM settings not configured. Please add Container ID and enable GTM.",
            };
        }
        ...
        if (!initialized) {
            return {
                success: false,
                message:
                    "Failed to initialize GTM. Please check your settings.",
            };
        }
        ...
        return {
            success: false,
            message: `GTM connection test failed: ${
