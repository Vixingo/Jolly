...
import { uploadFile, deleteFile, getFilePathFromUrl } from './storage-utils'
...
export async function uploadProductImages(
  ...
  try {
    ...
    if (!bucketExists) {
      console.error(`Bucket '${bucketName}' does not exist. Please run the storage-bucket-setup.sql script in your Supabase SQL Editor.`)
      return []
    }
    ...
    for (const [index, file] of Array.from(files).entries()) {
      ...
      if (!publicUrl) {
        console.error(`Failed to upload file ${file.name} to bucket ${bucketName}`)
        continue
      }
      ...
      if (insertError) {
        console.error('Error inserting product image record:', insertError)
        console.warn('Make sure you have run the product-images-setup.sql script in your Supabase SQL Editor')
    ...
    console.warn('Please run both storage-bucket-setup.sql and product-images-setup.sql scripts in your Supabase SQL Editor')
    return []
  }
}
 ...
 * @returns Boolean indicating success or failure
 */
export async function setPrimaryProductImage(imageId: string, productId: string): Promise<boolean> {
  try {
